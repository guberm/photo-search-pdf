...
- автоматическая установка официального Codex CLI через Windows Package Manager, если он отсутствует;
...
PhotoSearch PDF не использует OpenAI API key. Для вопросов нужен [Codex CLI или Codex desktop app](https://learn.chatgpt.com/docs/codex/cli). Технические команды вводить не нужно: приложение проверяет наличие Codex и, если его нет, после вашего подтверждения устанавливает официальный пакет `OpenAI.Codex` через встроенный Windows Package Manager (`winget`).

1. Откройте вкладку **Вопросы к документу**.
2. Нажмите **Подключить OpenAI**. При необходимости приложение само предложит установить Codex, затем откроет браузер для входа через ChatGPT.
3. После статуса «Подключено через подписку ChatGPT» выберите созданный PDF, введите вопрос и нажмите **Задать вопрос**.

Если пропустить настройку и сразу нажать **Задать вопрос**, тот же мастер установки и входа запустится автоматически. На редких Windows-системах без `winget` приложение откроет официальную инструкцию OpenAI.
